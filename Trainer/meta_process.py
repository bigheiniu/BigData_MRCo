...
def step_l2w_group_net(meta_trainer, main_net, main_opt,
                       meta_net, meta_opt,
                       clean_val_input,
                       train_aug_data, clean_train_input):
    ...
    loss_s, _ = meta_net(x_feature=s_feature,  loss_s=loss_s,
                         y_logits=logits_s, **train_aug_data)
    loss_train = main_net(**clean_train_input)[-1]
    ...
    outputs_s_p = main_net(**train_aug_data)
    ...
    loss_s_p, _ = meta_net(x_feature=s_p_feature, loss_s=loss_s_p,
                           y_logits=logits_s_p, **train_aug_data)
    loss_train = main_net(**clean_train_input)[-1]
    ...
    outputs_s_n = main_net(**train_aug_data)
    ...
    loss_s_n, _ = meta_net(x_feature=s_n_feature, loss_s=loss_s_n,
                           y_logits=logits_s_n, **train_aug_data)
    loss_train = main_net(**clean_train_input)[-1]
    ...
    main_opt.zero_grad()
    meta_trainer.manual_backward(loss_final, main_opt)
...
def step_l2w_group_net_previous(meta_trainer, main_net, main_opt,
    ...
    # update gw
    # gw_opt.zero_grad()
    meta_trainer.manual_backward(proxy_g, meta_opt)
    # proxy_g.backward()
    if meta_trainer.trainer.global_step % 25 == 0:  # don't make the tf file huge
        for name, p in meta_net.named_parameters():
            grad = p.grad
